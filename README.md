...
- 官方 product-facing provider smoke 已有 3 项真实通过：OCR、TEXT_SIMILARITY、EMBEDDING。
- Query Rewrite 已配置到 `READY`，但 live smoke 在当前本地 runtime 中 blocked；这不是官方 provider 不可用的证据，也不是 L3 blocker。
- Query Rewrite 当前产品兜底：云端 qwen3.5-plus rewrite 可用时优先，否则使用 local safe rewrite / direct retrieval fallback。
- Translation、TTS、Function Calling 仍按 seam-only / 后置处理；ASR Long 单独后置验证。
- 下一主线不是继续加功能或死磕 Query Rewrite，而是 App-level L3 云真机/真机学习闭环验证。
