...
class ProviderBackedOfficialRuntimeGateway(
    ...
    override fun rewriteQuery(query: String, summary: ProviderConfigSummary, now: Long): OfficialRuntimeResult<String> {
        if (!summary.officialProviders.queryRewriteConfigured) {
            return notConfigured(OfficialAiCapability.QUERY_REWRITE, query, "OFFICIAL_QUERY_REWRITE_NOT_CONFIGURED", now)
        }
        val result = queryRewriteProvider.rewrite(query)
        return when (result.status) {
            RetrievalStatus.SUCCESS -> used(OfficialAiCapability.QUERY_REWRITE, result.rewrittenQuery.ifBlank { query }, now)
            RetrievalStatus.CONFIG_MISSING -> appWiringPending(OfficialAiCapability.QUERY_REWRITE, query, "OFFICIAL_QUERY_REWRITE_ADAPTER_NOT_INJECTED", now)
            RetrievalStatus.FAILED -> failed(OfficialAiCapability.QUERY_REWRITE, query, "OFFICIAL_QUERY_REWRITE_FAILED", result.message, now)
        }
    }
    ...
    override fun embedText(text: String, summary: ProviderConfigSummary, now: Long): OfficialRuntimeResult<List<Double>> {
        if (!summary.officialProviders.embeddingConfigured) {
            return notConfigured(OfficialAiCapability.EMBEDDING, emptyList(), "OFFICIAL_EMBEDDING_NOT_CONFIGURED", now)
        }
        val result = embeddingProvider.embed(text)
        return when (result.status) {
            RetrievalStatus.SUCCESS -> {
                val vector = result.vector?.values.orEmpty().map { it.toDouble() }
                if (vector.isNotEmpty()) used(OfficialAiCapability.EMBEDDING, vector, now)
                else failed(OfficialAiCapability.EMBEDDING, emptyList(), "OFFICIAL_EMBEDDING_EMPTY", result.message, now)
            }
            RetrievalStatus.CONFIG_MISSING -> appWiringPending(OfficialAiCapability.EMBEDDING, emptyList(), "OFFICIAL_EMBEDDING_ADAPTER_NOT_INJECTED", now)
            RetrievalStatus.FAILED -> failed(OfficialAiCapability.EMBEDDING, emptyList(), "OFFICIAL_EMBEDDING_FAILED", result.message, now)
        }
    }
    ...
    override fun rankSimilarity(
        ...
        if (!summary.officialProviders.textSimilarityConfigured) {
            return notConfigured(OfficialAiCapability.TEXT_SIMILARITY, emptyList(), "OFFICIAL_TEXT_SIMILARITY_NOT_CONFIGURED", now)
        }
        if (candidates.isEmpty()) {
            return used(OfficialAiCapability.TEXT_SIMILARITY, emptyList(), now)
        }
        ...
        return when (result.status) {
            RetrievalStatus.SUCCESS -> {
                ...
                used(OfficialAiCapability.TEXT_SIMILARITY, scores, now)
            }
            RetrievalStatus.CONFIG_MISSING -> appWiringPending(OfficialAiCapability.TEXT_SIMILARITY, emptyList(), "OFFICIAL_TEXT_SIMILARITY_ADAPTER_NOT_INJECTED", now)
            RetrievalStatus.FAILED -> failed(OfficialAiCapability.TEXT_SIMILARITY, emptyList(), "OFFICIAL_TEXT_SIMILARITY_FAILED", result.message, now)
        }
    }
    ...
    override fun translate(
        ...
        if (!summary.officialProviders.translationConfigured) {
            return notConfigured(OfficialAiCapability.TRANSLATION, "", "OFFICIAL_TRANSLATION_NOT_CONFIGURED", now)
        }
        val result = translationProvider.translate(sourceText, sourceLanguage, targetLanguage)
        return when (result.status) {
            TranslationStatus.TRANSLATED -> used(OfficialAiCapability.TRANSLATION, result.translatedText, now)
            TranslationStatus.CONFIG_MISSING -> appWiringPending(OfficialAiCapability.TRANSLATION, "", "OFFICIAL_TRANSLATION_ADAPTER_NOT_INJECTED", now)
            TranslationStatus.FAILED -> failed(OfficialAiCapability.TRANSLATION, "", "OFFICIAL_TRANSLATION_FAILED", result.message, now)
            TranslationStatus.MANUAL -> localFallback(OfficialAiCapability.TRANSLATION, result.translatedText, "MANUAL_TRANSLATION_FALLBACK", now)
        }
    }
    ...
    override fun prepareTts(
        ...
        if (!summary.officialProviders.ttsConfigured) {
            return if (localTtsAvailable) {
                localFallback(OfficialAiCapability.TTS, "ANDROID_LOCAL_TTS", "OFFICIAL_TTS_NOT_CONFIGURED", now)
            } else {
                notConfigured(OfficialAiCapability.TTS, "NONE", "OFFICIAL_TTS_NOT_CONFIGURED", now)
            }
        }
        ...
        return when (result.status) {
            CourseEssenceAudioStatus.AUDIO_READY -> used(OfficialAiCapability.TTS, result.fileName ?: "official_tts_audio", now)
            CourseEssenceAudioStatus.SCRIPT_ONLY_CONFIG_MISSING -> if (localTtsAvailable) {
                appWiringPending(OfficialAiCapability.TTS, "ANDROID_LOCAL_TTS", "OFFICIAL_TTS_ADAPTER_NOT_INJECTED", now)
            } else {
                appWiringPending(OfficialAiCapability.TTS, "SCRIPT_ONLY", "OFFICIAL_TTS_ADAPTER_NOT_INJECTED", now)
            }
            CourseEssenceAudioStatus.FAILED -> failed(OfficialAiCapability.TTS, if (localTtsAvailable) "ANDROID_LOCAL_TTS" else "SCRIPT_ONLY", "OFFICIAL_TTS_FAILED", result.message, now)
        }
    }
    ...
    override fun proposeToolPlan(
        ...
        if (!summary.officialProviders.functionCallingConfigured) {
            return localFallback(OfficialAiCapability.FUNCTION_CALLING, plannedTools, "OFFICIAL_FUNCTION_CALLING_NOT_CONFIGURED", now)
        }
        val result = functionCallingProvider.propose(task, InternalToolName.entries)
        return when (result.status) {
            FunctionCallingStatus.TOOL_PROPOSED -> used(OfficialAiCapability.FUNCTION_CALLING, listOfNotNull(result.proposal?.toolName), now)
            FunctionCallingStatus.CONFIG_MISSING -> appWiringPending(OfficialAiCapability.FUNCTION_CALLING, plannedTools, "OFFICIAL_FUNCTION_CALLING_ADAPTER_NOT_INJECTED", now)
            FunctionCallingStatus.INVALID_TOOL -> failed(OfficialAiCapability.FUNCTION_CALLING, plannedTools, "OFFICIAL_FUNCTION_CALLING_INVALID_TOOL", result.message, now)
            FunctionCallingStatus.PARSE_FAILED -> failed(OfficialAiCapability.FUNCTION_CALLING, plannedTools, "OFFICIAL_FUNCTION_CALLING_PARSE_FAILED", result.message, now)
        }
    }
    ...
    private fun <T> used(capability: OfficialAiCapability, output: T, now: Long): OfficialRuntimeResult<T> =
        OfficialRuntimeResult(capability, OfficialRuntimeStatus.OFFICIAL_RUNTIME_USED, output, fallbackUsed = false, createdAt = now)

    private fun <T> localFallback(capability: OfficialAiCapability, output: T, code: String, now: Long): OfficialRuntimeResult<T> =
        OfficialRuntimeResult(capability, OfficialRuntimeStatus.LOCAL_FALLBACK_USED, output, errorCode = code, fallbackUsed = true, createdAt = now)

    private fun <T> notConfigured(capability: OfficialAiCapability, output: T, code: String, now: Long): OfficialRuntimeResult<T> =
        OfficialRuntimeResult(capability, OfficialRuntimeStatus.OFFICIAL_RUNTIME_NOT_CONFIGURED, output, errorCode = code, fallbackUsed = true, createdAt = now)

    private fun <T> appWiringPending(capability: OfficialAiCapability, output: T, code: String, now: Long): OfficialRuntimeResult<T> =
        OfficialRuntimeResult(capability, OfficialRuntimeStatus.OFFICIAL_APP_WIRING_PENDING, output, errorCode = code, fallbackUsed = true, createdAt = now)

    private fun <T> failed(capability: OfficialAiCapability, output: T, code: String, message: String, now: Long): OfficialRuntimeResult<T> =
        OfficialRuntimeResult(capability, OfficialRuntimeStatus.OFFICIAL_RUNTIME_FAILED, output, errorCode = code, errorMessage = message, fallbackUsed = true, createdAt = now)
}
