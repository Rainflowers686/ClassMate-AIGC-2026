...
| QUERY_REWRITE | Standardize study/retrieval queries | Official smoke PASS; v1.7 production factory injects the Vivo runtime adapter. It records `OFFICIAL_RUNTIME_USED` only when the runtime succeeds; otherwise local query planning fallback is used. |
| EMBEDDING | Lesson/evidence/KP/question index records | Official smoke PASS; v1.7 production factory injects the Vivo runtime adapter. It stores official vectors only when the runtime succeeds; otherwise the persistent local lexical index remains the fallback. |
...
| TEXT_SIMILARITY | Evidence match, similar KP/question attribution | Official smoke PASS; v1.7 production factory injects the Vivo runtime adapter. It stores official score provenance only when the runtime succeeds; otherwise local similarity fallback is used. |
