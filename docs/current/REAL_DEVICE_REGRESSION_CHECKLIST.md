# Real Device Regression Checklist - 1.14.3
...
- 系统语音识别不可用：录音卡片应显示“打开语音设置”，同时允许停止保存录音并手动粘贴转写。
- 题目反馈：提交“不准确/证据不对”后应出现即时优化提示，后续微测候选包含替换题。
- 知识点反馈：提交“不清楚”后应重写摘要并把该知识点放入复习任务。
- 复习页：应显示本课核心知识点、相关知识点和证据摘录，而不是技术统计主文案。
