...
1. ASR 文档评审与 Provider 设计：先读 `1739`、`1740`、`1738` 的 `content.md`、`tables.md`、`code_blocks.txt`。
2. `VivoAsrProvider` smoke：优先 `1739` 长语音转写；用 `1740` 长语音听写做实时/长连接对照；`1738` 实时短语音后置。
3. `TranscriptDraft` / 用户确认 / `CourseAnalysis` 链路：ASR 结果必须进入可编辑草稿，用户确认后才进入知识库。
4. `VivoOcrProvider` smoke：基于 `1737` 通用 OCR；图片/拍照走 OCR + 端侧多模态双轨，不互相替代。
5. Retrieval provider：先用 `2060` 文本相似度做轻量 evidence matching；再用 `2061` 查询改写增强 Ask；最后用 `1734` 文本向量支持课程库搜索和知识聚类。
6. Evidence-grounded Ask：所有模型回答必须回到 evidence quotes / source refs，无法定位证据时降级为 partial / not_found。
7. Practice -> Weakness Hub -> Review：把 Ask、Quiz、Practice 的错误与反馈转为复习优先级，而不是只生成一次性总结。
8. Function calling 评估：只在 evidence、quiz、review、export 工具权限可控后再引入。
9. TTS / Flow Companion：复习陪伴和朗读后置，避免在 ASR/OCR 之前分散主线。
10. 图片生成 / 视频生成 / LBS / 声音复刻等能力后置；不做第三方平台视频爬取。
